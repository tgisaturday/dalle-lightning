...
class VQVAE2(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):         
        ...
        self.log("train/ae_loss", recon_loss, prog_bar=True, logger=False, on_step=True, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val/ae_loss", recon_loss, prog_bar=True, logger=False, on_step=True, on_epoch=True)
        log_dict = dict()      
        if x.shape[1] > 3:
            # colorize with random projection
            assert xrec.shape[1] > 3
